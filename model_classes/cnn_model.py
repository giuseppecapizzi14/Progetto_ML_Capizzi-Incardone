...
class EmovoCNN(nn.Module):
    def __init__(self, waveform_size: int, num_classes: int, dropout: float, device: torch.device):
        def output_dimensions(input_size: int, padding: int, kernel_size: int, stride: int) -> int:
        ...
        conv1_stride = 5
        self.sample_len = output_dimensions(waveform_size, 0, conv1_kernel_size, conv1_stride)

        # Primo strato pooling
        pool1_kernel_size = 3
        pool1_stride = 2
        self.sample_len = output_dimensions(self.sample_len, 0, pool1_kernel_size, pool1_stride)
        ...
        conv2_kernel_size = 3
        conv2_stride = 2
        self.sample_len = output_dimensions(self.sample_len, 0, conv2_kernel_size, conv2_stride)

        # Secondo strato pooling
        pool2_kernel_size = 3
        pool2_stride = 2
        self.sample_len = output_dimensions(self.sample_len, 0, pool2_kernel_size, pool2_stride)
        ...
        conv3_kernel_size = 3
        conv3_stride = 2
        self.sample_len = output_dimensions(self.sample_len, 0, conv3_kernel_size, conv3_stride)

        # Terzo strato pooling
        pool3_kernel_size = 3
        pool3_stride = 2
        self.sample_len = output_dimensions(self.sample_len, 0, pool3_kernel_size, pool3_stride)
